    ...
    def domain_randomization(self):
        ...
        for z in range(2):
            self.mj_data.qpos[self.nj + 1 + 7 * 2 + z * 7 + 0] += (
                10.0 * (np.random.random() - 0.5) * 0.002
            )
            self.mj_data.qpos[self.nj + 1 + 7 * 2 + z * 7 + 1] += (
                10.0 * (np.random.random() - 0.5) * 0.002
            )

            random_angle = np.random.uniform(-np.pi / 4, np.pi / 4)  # 调整角度范围

            
            # 获取当前物体的四元数
            current_quat = self.mj_data.qpos[
                self.nj + 1 + 7 * 2 + z * 7 + 3 :
                self.nj + 1 + 7 * 2 + z * 7 + 3+4
            ]

            current_euler = R.from_quat(current_quat).as_euler("xyz", degrees=False)
    
            # 组合旋转（当前旋转 * 随机z轴旋转）
            new_euler = current_euler + np.array([random_angle,0,0])
            
            # 获取新的四元数，并归一化
            new_quat = R.from_euler("xyz",new_euler).as_quat()
        ...
        camera.pos[:] = self.camera_1_pose[0] + 2.*(np.random.random(3) - 0.5) * 0.02
        euler = Rotation.from_quat(self.camera_1_pose[1][[1,2,3,0]]).as_euler("xyz", degrees=False) + 2.*(np.random.random(3) - 0.5) * 0.1
...
if __name__ == "__main__":
    ...
    # save_dir = "/media/hdl/My_Passport/Tsinghua_air/data_create/state4"
    save_dir = os.path.join(DISCOVERSE_ROOT_DIR, "data/state4_random1000_2")
        ...
        try:
            if stm.trigger():
                ...
                if stm.state_idx == 0: # 伸到拱桥上方
                    ...
                    sim_node.target_control[:6] = arm_fik.properIK(tmat_tgt_local[:3,3], trmat, sim_node.mj_data.qpos[:6])
                    sim_node.target_control[6] = 1
                elif stm.state_idx == 2: # 对齐长方体姿态
                    tmat_block1 = get_body_tmat(sim_node.mj_data, "block1_green")
                    tmat_block1[:3, 3] = tmat_block1[:3, 3] + np.array([0, 0, 0.12])
                    R_block1 = tmat_block1[:3, :3]
                    tmat_tgt_local = tmat_armbase_2_world @ tmat_block1
                    # 更新机械臂末端执行器的目标姿态
                    euler = Rotation.from_matrix(R_block1).as_euler("xyz", degrees=False)
                    block_angle = euler[2]
                    trmat = Rotation.from_euler("xyz", [0., np.pi/2, block_angle], degrees=False).as_matrix()
                    sim_node.target_control[:6] = arm_fik.properIK(tmat_tgt_local[:3,3], trmat, sim_node.mj_data.qpos[:6])
                    ...
                    tmat_block2 = get_body_tmat(sim_node.mj_data, "block2_green")
                    tmat_block2[:3, 3] = tmat_block2[:3, 3] + np.array([0, 0, 0.12])
                    R_block2 = tmat_block2[:3, :3]
                    tmat_tgt_local = tmat_armbase_2_world @ tmat_block2
                    # 更新机械臂末端执行器的目标姿态
                    euler = Rotation.from_matrix(R_block2).as_euler("xyz", degrees=False)
                    block_angle = euler[2]
                    trmat = Rotation.from_euler("xyz", [0., np.pi/2, block_angle], degrees=False).as_matrix()
                    sim_node.target_control[:6] = arm_fik.properIK(tmat_tgt_local[:3,3], trmat, sim_node.mj_data.qpos[:6])
                 ...
                 # 1
                    ...
                    sim_node.target_control[:6] = arm_fik.properIK(tmat_tgt_local[:3,3], trmat, sim_node.mj_data.qpos[:6])
                    sim_node.target_control[6] = 1
                # elif stm.state_idx == 23:
                #     tmat_block = get_body_tmat(sim_node.mj_data, "block_purple1")
                #     tmat_block[:3, 3] = tmat_block[:3, 3] + np.array([0, 0, 0.12])
                #     R_block = tmat_block[:3, :3]
                #     tmat_tgt_local = tmat_armbase_2_world @ tmat_block
                #     # 更新机械臂末端执行器的目标姿态
                #     euler = Rotation.from_matrix(R_block).as_euler("xyz", degrees=False)
                #     block_angle = euler[2]
                #     trmat = Rotation.from_euler("xyz", [0., np.pi/2, block_angle], degrees=False).as_matrix()
                #     sim_node.target_control[:6] = arm_fik.properIK(tmat_tgt_local[:3,3], trmat, sim_node.mj_data.qpos[:6])
                elif stm.state_idx == 24: # 伸到立方体
                    tmat_block = get_body_tmat(sim_node.mj_data, "block_purple1")
                    tmat_block[:3, 3] = tmat_block[:3, 3] + np.array([0, 0, 0.03])
                    tmat_tgt_local = tmat_armbase_2_world @ tmat_block
                    sim_node.target_control[:6] = arm_fik.properIK(tmat_tgt_local[:3,3], trmat, sim_node.mj_data.qpos[:6])
                elif stm.state_idx == 25: # 抓住立方体
                    sim_node.target_control[6] = 0.1
                elif stm.state_idx == 26: # 抓稳立方体
                    sim_node.delay_cnt = int(0.35/sim_node.delta_t)
                elif stm.state_idx == 27: # 提起立方体
                    # trmat = Rotation.from_euler("xyz", [0., np.pi/2, 0.], degrees=False).as_matrix()
                    tmat_tgt_local[2,3] += 0.09
                    sim_node.target_control[:6] = arm_fik.properIK(tmat_tgt_local[:3,3], trmat, sim_node.mj_data.qpos[:6])
                elif stm.state_idx == 28: # 把立方体放到长方体上方
                    tmat_block2 = get_body_tmat(sim_node.mj_data, "block2_green")
                    tmat_block2[:3,3] = tmat_block2[:3, 3] + np.array([0, 0, 0.04+0.031*1])
                    tmat_tgt_local = tmat_armbase_2_world @ tmat_block2
                    sim_node.target_control[:6] = arm_fik.properIK(tmat_tgt_local[:3,3], trmat, sim_node.mj_data.qpos[:6])               
                elif stm.state_idx == 29: # 把立方体放到长方体上侧
                    tmat_tgt_local[2,3] -= 0.01
                    sim_node.target_control[:6] = arm_fik.properIK(tmat_tgt_local[:3,3], trmat, sim_node.mj_data.qpos[:6])
                elif stm.state_idx == 30: # 松开方块
                    sim_node.target_control[6] = 1
                elif stm.state_idx == 31: # 抬升高度
                    tmat_tgt_local[2,3] += 0.02
                    sim_node.target_control[:6] = arm_fik.properIK(tmat_tgt_local[:3,3], trmat, sim_node.mj_data.qpos[:6])


                # 2
                elif stm.state_idx == 32: # 伸到立方体上方
                ...
                #     trmat = Rotation.from_euler("xyz", [0., np.pi/2, block_angle], degrees=False).as_matrix()
                #     sim_node.target_control[:6] = arm_fik.properIK(tmat_tgt_local[:3,3], trmat, sim_node.mj_data.qpos[:6])
                elif stm.state_idx == 34: # 伸到立方体
                    tmat_block = get_body_tmat(sim_node.mj_data, "block_purple4")
                    tmat_block[:3, 3] = tmat_block[:3, 3] + np.array([0, 0, 0.03])
                    tmat_tgt_local = tmat_armbase_2_world @ tmat_block
                    sim_node.target_control[:6] = arm_fik.properIK(tmat_tgt_local[:3,3], trmat, sim_node.mj_data.qpos[:6])
                elif stm.state_idx == 35: # 抓住立方体
                    sim_node.target_control[6] = 0.1
                elif stm.state_idx == 36: # 抓稳立方体
                    sim_node.delay_cnt = int(0.35/sim_node.delta_t)
                elif stm.state_idx == 37: # 提起立方体
                    # trmat = Rotation.from_euler("xyz", [0., np.pi/2, 0.], degrees=False).as_matrix()
                    tmat_tgt_local[2,3] += 0.09
                    sim_node.target_control[:6] = arm_fik.properIK(tmat_tgt_local[:3,3], trmat, sim_node.mj_data.qpos[:6])
                elif stm.state_idx == 38: # 把立方体放到长方体上方
                    tmat_block1 = get_body_tmat(sim_node.mj_data, "block1_green")
                    tmat_block1[:3,3] = tmat_block1[:3, 3] + np.array([0, 0, 0.04+0.031*1])
                    tmat_tgt_local = tmat_armbase_2_world @ tmat_block1
                    sim_node.target_control[:6] = arm_fik.properIK(tmat_tgt_local[:3,3], trmat, sim_node.mj_data.qpos[:6])               
                elif stm.state_idx == 39: # 把立方体放到长方体上侧
                    tmat_tgt_local[2,3] -= 0.01
                    sim_node.target_control[:6] = arm_fik.properIK(tmat_tgt_local[:3,3], trmat, sim_node.mj_data.qpos[:6])
                elif stm.state_idx == 40: # 松开方块
                    sim_node.target_control[6] = 1
                elif stm.state_idx == 41: # 抬升高度
                    tmat_tgt_local[2,3] += 0.02
                    sim_node.target_control[:6] = arm_fik.properIK(tmat_tgt_local[:3,3], trmat, sim_node.mj_data.qpos[:6])
                
                #3
                elif stm.state_idx == 42: # 伸到立方体上方
                    tmat_block = get_body_tmat(sim_node.mj_data, "block_purple2")
                    tmat_block[:3, 3] = tmat_block[:3, 3] + np.array([0, 0, 0.12])
                    tmat_tgt_local = tmat_armbase_2_world @ tmat_block
                    sim_node.target_control[:6] = arm_fik.properIK(tmat_tgt_local[:3,3], trmat, sim_node.mj_data.qpos[:6])
                    sim_node.target_control[6] = 1
                # elif stm.state_idx == 43:
                #     tmat_block = get_body_tmat(sim_node.mj_data, "block_purple2")
                #     tmat_block[:3, 3] = tmat_block[:3, 3] + np.array([0, 0, 0.12])
                #     R_block = tmat_block[:3, :3]
                #     tmat_tgt_local = tmat_armbase_2_world @ tmat_block
                ...
                #     trmat = Rotation.from_euler("xyz", [0., np.pi/2, block_angle], degrees=False).as_matrix()
                #     sim_node.target_control[:6] = arm_fik.properIK(tmat_tgt_local[:3,3], trmat, sim_node.mj_data.qpos[:6])
                elif stm.state_idx == 44: # 伸到立方体
                    tmat_block = get_body_tmat(sim_node.mj_data, "block_purple2")
                    tmat_block[:3, 3] = tmat_block[:3, 3] + np.array([0, 0, 0.03])
                    tmat_tgt_local = tmat_armbase_2_world @ tmat_block
                    sim_node.target_control[:6] = arm_fik.properIK(tmat_tgt_local[:3,3], trmat, sim_node.mj_data.qpos[:6])
                elif stm.state_idx == 45: # 抓住立方体
                    sim_node.target_control[6] = 0.1
                ...
                #     trmat = Rotation.from_euler("xyz", [0., np.pi/2, block_angle], degrees=False).as_matrix()
                #     sim_node.target_control[:6] = arm_fik.properIK(tmat_tgt_local[:3,3], trmat, sim_node.mj_data.qpos[:6])
                elif stm.state_idx == 54: # 伸到立方体
                    tmat_block = get_body_tmat(sim_node.mj_data, "block_purple5")
                    tmat_block[:3, 3] = tmat_block[:3, 3] + np.array([0, 0, 0.03])
                    tmat_tgt_local = tmat_armbase_2_world @ tmat_block
                    sim_node.target_control[:6] = arm_fik.properIK(tmat_tgt_local[:3,3], trmat, sim_node.mj_data.qpos[:6])
                elif stm.state_idx == 55: # 抓住立方体
                    sim_node.target_control[6] = 0.1
                elif stm.state_idx == 56: # 抓稳立方体
                    sim_node.delay_cnt = int(0.35/sim_node.delta_t)
                elif stm.state_idx == 57: # 提起立方体
                    # trmat = Rotation.from_euler("xyz", [0., np.pi/2, 0.], degrees=False).as_matrix()
                    tmat_tgt_local[2,3] += 0.09
                    sim_node.target_control[:6] = arm_fik.properIK(tmat_tgt_local[:3,3], trmat, sim_node.mj_data.qpos[:6])
                elif stm.state_idx == 58: # 把立方体放到长方体上方
                    tmat_block1 = get_body_tmat(sim_node.mj_data, "block1_green")
                    tmat_block1[:3,3] = tmat_block1[:3, 3] + np.array([0, 0, 0.04+0.031*2])
                    tmat_tgt_local = tmat_armbase_2_world @ tmat_block1
                    sim_node.target_control[:6] = arm_fik.properIK(tmat_tgt_local[:3,3], trmat, sim_node.mj_data.qpos[:6])               
                elif stm.state_idx == 59: # 把立方体放到长方体上侧
                    tmat_tgt_local[2,3] -= 0.01
                    sim_node.target_control[:6] = arm_fik.properIK(tmat_tgt_local[:3,3], trmat, sim_node.mj_data.qpos[:6])
                elif stm.state_idx == 60: # 松开方块
                    sim_node.target_control[6] = 1
                elif stm.state_idx == 61: # 抬升高度
                    tmat_tgt_local[2,3] += 0.02
                    sim_node.target_control[:6] = arm_fik.properIK(tmat_tgt_local[:3,3], trmat, sim_node.mj_data.qpos[:6])
                
                #5
                elif stm.state_idx == 62: # 伸到立方体上方
                ...
                #     trmat = Rotation.from_euler("xyz", [0., np.pi/2, block_angle], degrees=False).as_matrix()
                #     sim_node.target_control[:6] = arm_fik.properIK(tmat_tgt_local[:3,3], trmat, sim_node.mj_data.qpos[:6])
                elif stm.state_idx == 64: # 伸到立方体
                    tmat_block = get_body_tmat(sim_node.mj_data, "block_purple3")
                    tmat_block[:3, 3] = tmat_block[:3, 3] + np.array([0, 0, 0.03])
                    tmat_tgt_local = tmat_armbase_2_world @ tmat_block
                    sim_node.target_control[:6] = arm_fik.properIK(tmat_tgt_local[:3,3], trmat, sim_node.mj_data.qpos[:6])
                elif stm.state_idx == 65: # 抓住立方体
                    sim_node.target_control[6] = 0.1
                elif stm.state_idx == 66: # 抓稳立方体
                    sim_node.delay_cnt = int(0.35/sim_node.delta_t)
                elif stm.state_idx == 67: # 提起立方体
                    # trmat = Rotation.from_euler("xyz", [0., np.pi/2, 0.], degrees=False).as_matrix()
                    tmat_tgt_local[2,3] += 0.09
                    sim_node.target_control[:6] = arm_fik.properIK(tmat_tgt_local[:3,3], trmat, sim_node.mj_data.qpos[:6])
                elif stm.state_idx == 68: # 把立方体放到长方体上方
                    tmat_block2 = get_body_tmat(sim_node.mj_data, "block2_green")
                    tmat_block2[:3,3] = tmat_block2[:3, 3] + np.array([0, 0, 0.04+0.031*3])
                    tmat_tgt_local = tmat_armbase_2_world @ tmat_block2
                    sim_node.target_control[:6] = arm_fik.properIK(tmat_tgt_local[:3,3], trmat, sim_node.mj_data.qpos[:6])               
                elif stm.state_idx == 69: # 把立方体放到长方体上侧
                    tmat_tgt_local[2,3] -= 0.01
                    sim_node.target_control[:6] = arm_fik.properIK(tmat_tgt_local[:3,3], trmat, sim_node.mj_data.qpos[:6])
                elif stm.state_idx ==70: # 松开方块
                    sim_node.target_control[6] = 1
                elif stm.state_idx == 71: # 抬升高度
                    tmat_tgt_local[2,3] += 0.02
                    sim_node.target_control[:6] = arm_fik.properIK(tmat_tgt_local[:3,3], trmat, sim_node.mj_data.qpos[:6])
                ...
                #     trmat = Rotation.from_euler("xyz", [0., np.pi/2, block_angle], degrees=False).as_matrix()
                #     sim_node.target_control[:6] = arm_fik.properIK(tmat_tgt_local[:3,3], trmat, sim_node.mj_data.qpos[:6])
                elif stm.state_idx == 74: # 伸到立方体
                    tmat_block = get_body_tmat(sim_node.mj_data, "block_purple6")
                    tmat_block[:3, 3] = tmat_block[:3, 3] + np.array([0, 0, 0.03])
                    tmat_tgt_local = tmat_armbase_2_world @ tmat_block
                    sim_node.target_control[:6] = arm_fik.properIK(tmat_tgt_local[:3,3], trmat, sim_node.mj_data.qpos[:6])
                elif stm.state_idx == 75: # 抓住立方体
                    sim_node.target_control[6] = 0.1
                elif stm.state_idx == 76: # 抓稳立方体
                    sim_node.delay_cnt = int(0.35/sim_node.delta_t)
                elif stm.state_idx == 77: # 提起立方体
                    # trmat = Rotation.from_euler("xyz", [0., np.pi/2, 0.], degrees=False).as_matrix()
                    tmat_tgt_local[2,3] += 0.09
                    sim_node.target_control[:6] = arm_fik.properIK(tmat_tgt_local[:3,3], trmat, sim_node.mj_data.qpos[:6])
                elif stm.state_idx == 78: # 把立方体放到长方体上方
                    tmat_block1 = get_body_tmat(sim_node.mj_data, "block1_green")
                    tmat_block1[:3,3] = tmat_block1[:3, 3] + np.array([0, 0, 0.04+0.031*3])
                    tmat_tgt_local = tmat_armbase_2_world @ tmat_block1
                    sim_node.target_control[:6] = arm_fik.properIK(tmat_tgt_local[:3,3], trmat, sim_node.mj_data.qpos[:6])               
                elif stm.state_idx == 79: # 把立方体放到长方体上侧
                    tmat_tgt_local[2,3] -= 0.01
                    sim_node.target_control[:6] = arm_fik.properIK(tmat_tgt_local[:3,3], trmat, sim_node.mj_data.qpos[:6])
                elif stm.state_idx == 80: # 松开方块
                    sim_node.target_control[6] = 1
                elif stm.state_idx == 81: # 抬升高度
                    tmat_tgt_local[2,3] += 0.02
                    sim_node.target_control[:6] = arm_fik.properIK(tmat_tgt_local[:3,3], trmat, sim_node.mj_data.qpos[:6])
