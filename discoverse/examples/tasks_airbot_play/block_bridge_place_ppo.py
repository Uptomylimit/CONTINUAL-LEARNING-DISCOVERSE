...
class SimNode(AirbotPlayTaskBase):
    def __init__(self, config: AirbotPlayCfg):
        ...
        self.height = cfg.render_set["height"]
        self.width = cfg.render_set["width"]
        ...
        # 添加bridge初始质心位置，作为判断bridge有没有被碰倒
        bridge_names = ["bridge1","bridge2"]
        for body_name in bridge_names:
    ...
    def check_success(self):
        ...
        # print(((abs(tmat_block1[2, 2]) < 0.001) 
        #         and (abs(abs(tmat_bridge1[1,3] - tmat_bridge2[1,3]) - 0.03) <= 0.002) 
        #         and (abs(tmat_block2[2, 2]) < 0.001)))
        return ((abs(tmat_block1[2, 2]) < 0.001) 
                and (abs(abs(tmat_bridge1[1,3] - tmat_bridge2[1,3]) - 0.03) <= 0.002) 
                and (abs(tmat_block2[2, 2]) < 0.001)
                and (abs(abs(tmat_bridge1[0,3] - tmat_block1[0,3])) <= 0.002)
                and (abs(abs(tmat_bridge1[0,3] - tmat_block2[0,3])) <= 0.002))
        # return ((abs(tmat_block1[2, 2]) < 0.001) 
        #         and (abs(abs(tmat_bridge1[1,3] - tmat_bridge2[1,3]) - 0.03) <= 0.002) 
        #         and (abs(tmat_block2[2, 2]) < 0.001))
    ...
    def getReward(self):
        ...
        remain_body_names = ["bridge1","bridge2","block_purple1", "block_purple2", "block_purple3", "block_purple4", "block_purple5", "block_purple6"]
        flag = True

        # # 6个紫色方块位置不变：
        # for z in range(6):
        #     if np.sqrt(np.linalg.norm(self.mj_data.qpos[self.nj + 1 + 7 * 4 + z * 7: self.nj + 1 + 7 * 4 + z * 7+3]))>0.01:
        #         flag = False
        # # 2个bridge位置不变
        # for z in range(2):
        #     if np.sqrt(np.linalg.norm(self.mj_data.qpos[self.nj + 1 + z * 7: self.nj + 1 + z * 7+3]))>0.01:
        #         flag = False
        
        for body_name in remain_body_names:
            body_id = self.mj_model.body(body_name).id
            # print(body_name,self.original_ipos[body_name] - self.mj_model.body_ipos[body_id])
            if np.sqrt(np.linalg.norm(self.original_ipos[body_name] - self.mj_model.body_ipos[body_id]))>0.1:
                flag = False
        if flag:
             id_bridge1 = self.mj_model.body("bridge1").id
             id_block1_green = self.mj_model.body("block1_green").id
             id_block2_green = self.mj_model.body("block2_green").id
             reward = 10*((0.09-abs(self.mj_model.body_ipos[id_block1_green][0] - self.mj_model.body_ipos[id_bridge1][0])) + 
                       (0.09-abs(self.mj_model.body_ipos[id_block2_green][0] - self.mj_model.body_ipos[id_bridge1][0])))
        else:
            reward = -1
        # print("reward:",reward)
        return reward
...
if __name__ == "__main__":
    ...
    save_dir = os.path.join(DISCOVERSE_ROOT_DIR, "data/block_bridge_place_1")
