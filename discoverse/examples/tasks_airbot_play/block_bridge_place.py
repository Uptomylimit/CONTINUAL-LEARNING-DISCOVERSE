...
class SimNode(AirbotPlayTaskBase):
    ...
    def domain_randomization(self):
        ...
        for z in range(2):
            self.mj_data.qpos[self.nj+1+7*2+z*7+0] += 2.*(np.random.random() - 0.5) * 0.001
            self.mj_data.qpos[self.nj+1+7*2+z*7+1] += 2.*(np.random.random() - 0.5) * 0.001

        # 随机 10个紫色方块位置

        for z in range(6):
            self.mj_data.qpos[self.nj+1+7*4+z*7+0] += 2.*(np.random.random() - 0.5) * 0.001
            self.mj_data.qpos[self.nj+1+7*4+z*7+1] += 2.*(np.random.random() - 0.5) * 0.001
    ...
    def check_success(self):
        ...
        return (abs(tmat_block1[2, 2]) < 0.001) and (abs(abs(tmat_bridge1[1,3] - tmat_bridge2[1,3]) - 0.03) <= 0.002) and (abs(tmat_block2[2, 2]) < 0.001) and np.hypot(tmat_block1[0, 3] - tmat_block01[0, 3], tmat_block2[1, 3] - tmat_block02[1, 3]) < 0.11
...
# cfg.gs_model_dict["arm_base"] = "my_airbot_play/arm_base.ply"
# cfg.gs_model_dict["link1"] = "my_airbot_play/link1.ply"
# cfg.gs_model_dict["link2"] = "my_airbot_play/link2.ply"
# cfg.gs_model_dict["link3"] = "my_airbot_play/link3.ply"
# cfg.gs_model_dict["link4"] = "my_airbot_play/link4.ply"
# cfg.gs_model_dict["link5"] = "my_airbot_play/link5.ply"
# cfg.gs_model_dict["link6"] = "my_airbot_play/link6.ply"
# cfg.gs_model_dict["left"] = "my_airbot_play/left.ply"
