...
class RDT(nn.Module):
    ...
    def forward(self, x, t, img_c, img_mask=None):
        ...
        freq not supported for now: (B,), a scalar indicating control frequency.
        t: (B,) or (1,), diffusion timesteps.
        lang_c not supported for now: (B, L_lang, D) or None, language condition tokens (variable length),
            dimension D is assumed to be the same as the hidden size.
