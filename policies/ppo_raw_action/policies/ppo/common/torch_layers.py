...
from policies.ppo.common.preprocessing import get_flattened_obs_dim, is_image_space
...
class Ncam_NatureCNN(BaseFeaturesExtractor):
    """
    CNN from DQN Nature paper:
        Mnih, Volodymyr, et al.
        "Human-level control through deep reinforcement learning."
    ...
    def __init__(
        ...
        super().__init__(observation_space, features_dim)
        self.height = observation_space.shape[2]
        self.width = observation_space.shape[3]
        self.n_input_channels = observation_space.shape[0]*observation_space.shape[1]
        # We assume CxHxW images (channels first)
        # Re-ordering will be done by pre-preprocessing or wrapper
        assert is_image_space(observation_space, check_channels=False, normalized_image=normalized_image), (
            "You should use NatureCNN "
        ...
        # Compute shape by doing one forward pass
        with th.no_grad():
            tem = observation_space.sample()[None]
            # print("-------------------------------")
            # print("self.cnn(x).shape",self.cnn(th.as_tensor(
            #     tem.reshape(tem.shape[0],self.n_input_channels,observation_space.shape[2],observation_space.shape[3])).float()).shape
# )
            n_flatten = self.cnn(th.as_tensor(
                tem.reshape(tem.shape[0],self.n_input_channels,observation_space.shape[2],observation_space.shape[3])).float()).shape[1]

        self.linear = nn.Sequential(nn.Linear(n_flatten, features_dim), nn.ReLU())

    def forward(self, observations: th.Tensor) -> th.Tensor:

        observations = observations.reshape(observations.shape[0],self.n_input_channels,self.height,self.width)
        # print("-------------------------------")
        # print("observations.reshape",observations.shape)
...
class MlpExtractor(nn.Module):
    ...
    def __init__(
        ...
        if self.act_policy_config is None:
    ...
    def forward(self, features: th.Tensor) -> tuple[th.Tensor, th.Tensor]:
        ...
        if self.act_policy_config is None:
            ...
            return th.tensor([0]), self.forward_critic(features)
...
class CombinedExtractor(BaseFeaturesExtractor):
    ...
    def __init__(
        ...
        for key, subspace in observation_space.spaces.items():
            if is_image_space(subspace, normalized_image=normalized_image):
                ...
                extractors[key] = Ncam_NatureCNN(subspace, features_dim=cnn_output_dim, normalized_image=normalized_image)
    ...
    def forward(self, observations: TensorDict) -> th.Tensor:
        # print("combinedextractor extract!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
        encoded_tensor_list = []

        for key, extractor in self.extractors.items():
            # print("437extractor(observations[key]).shape",extractor(observations[key]).shape)
