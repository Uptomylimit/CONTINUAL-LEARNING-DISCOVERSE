...
from policies.ppo.common.preprocessing import get_action_dim, is_image_space, maybe_transpose, preprocess_obs
from policies.ppo.common.torch_layers import (
...
from policies.ppo.common.utils import get_device, is_vectorized_observation, obs_as_tensor
...
class BaseModel(nn.Module):
    ...
    def extract_features(self, obs: PyTorchObs, features_extractor: BaseFeaturesExtractor) -> th.Tensor:
        ...
        # print("----------------------------")
        # print("obs.shape",obs["images"].shape)
        # print("preprocessed_obs.shape",preprocessed_obs["images"].shape)
...
class BasePolicy(BaseModel, ABC):
    ...
    def predict(
        ...
        with th.no_grad():
            ...
            # print("----------------------------")
            # print("basepolicy predict:", actions)
        # Convert to numpy, and reshape to the original action shape
        actions = actions.cpu().numpy().reshape((-1, *self.action_space.shape))  # type: ignore[misc, assignment]

        # print("----------------------------")
        # print("basepolicy predict2:", actions)

        # unscale and clip all in env
...
class ACT_ActorCriticPolicy(BasePolicy):
    """
    Policy class for actor-critic algorithms (has both policy and value prediction).
    Used by A2C, PPO and the likes.
    ...
    def __init__(
        ...
        features_extractor_kwargs: Optional[dict[str, Any]] = None,
        share_features_extractor: bool = True,
        # normalize_images: bool = True, 在输入进来之前处理
    ...
    def _build_mlp_extractor(self) -> None:
        ...
        Create the policy and value networks.
        Part of the layers can be shared.
        对于act，policy_net直接采用act的网络，无需feature_extractor，value_net采用ppo原来的网络
        """
        # Note: If net_arch is None and some features extractor is used,
        #       net_arch here is an empty list and mlp_extractor does not
        #       really contain any layers (acts like an identity module).
        self.mlp_extractor = MlpExtractor(
            ...
            activation_fn=self.activation_fn,
            device=self.device,
            act_policy_config=self.act_policy_config
        )
    ...
    def _build(self, lr_schedule: Schedule) -> None:
        ...
        #     print("AC_parameters:",name)
        #     print("AC_parameters:",param)
        print("Initialize act_actorcritic fucking successful!!!!!!!!!!!!!!!!!!!!!!")
    ...
    def forward(self, obs: th.Tensor, deterministic: bool = False) -> tuple[th.Tensor, th.Tensor, th.Tensor]:
        ...
        # print("policies.py 1017",features.shape)
        ...
        raw_actions = self.policy_net.no_temporal_agg_calculate(obs["qpos"],obs["images"])
        ...
        if isinstance(self.action_dist, DiagGaussianDistribution):
            ...
            distribution = self.action_dist.proba_distribution(raw_actions, self.log_std)

        # distribution = self._get_action_dist_from_latent(latent_pi)
        sample_raw_actions = distribution.get_actions(deterministic=deterministic)

        log_prob = distribution.log_prob(sample_raw_actions)
        # print("actions:",actions)
        # print("values:",values)
        # print("log_prob:",log_prob)
        actions = sample_raw_actions.clone()
        actions = actions.reshape((-1, self.act_policy_config["chunk_size"], self.act_policy_config["action_dim"]))
        
        a_hat_one = self.policy_net.temporal_ensembler.update(actions)
        # print("a_hat_one:", a_hat_one)

        actions[0][0] = a_hat_one
        ...
        return actions, sample_raw_actions, values, log_prob
    ...
    def evaluate_actions_for_NAN(self, obs, raw_actions) -> tuple[th.Tensor, th.Tensor, Optional[th.Tensor]]:
        """
