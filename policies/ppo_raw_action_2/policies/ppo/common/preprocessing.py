...
def is_image_space(
    ...
    elif isinstance(observation_space, spaces.Box) and len(observation_space.shape) == 4:
        # Check the type
        # if check_dtype and observation_space.dtype != np.uint8:
        #     return False

        # Check the value range
        incorrect_bounds = np.any(observation_space.low != 0) or np.any(observation_space.high != 1)
        if check_bounds and incorrect_bounds:
            return False
